...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y)
{
  ...
  // Verifica se o caractere é maiúsculo, minúsculo ou número
  if (c >= 'A' && c <= 'Z')
  {
  ...
  {
    // Se o caractere não for mapeado, apenas retorna
    return;
  }

  // Depuração: Imprime o índice calculado
  printf("Caractere: %c, Índice: %d\n", c, index);

  // Verifica se o índice está dentro dos limites da fonte
  if (index >= sizeof(font))
  {
    printf("Índice fora do limite! Índice: %d\n", index);
    return;
  }

  // Depuração: Imprime os valores dos bytes da fonte para o caractere
  for (uint8_t i = 0; i < 8; ++i)
  {
    uint8_t line = font[index + i];
    printf("Linha %d: 0x%02X\n", i, line); // Imprime o valor hexadecimal de cada linha
